function [ObjFunVal]=StegerObjFun_Entc(x,xw,xi)
% This function calculates the value of the objection function to fit
% Stegen Entocentric Camera model
...
tiltAngle1=x(16);   % rho, in virtual plane
tiltAngle2=x(17);   % tau, lens tilt angle
...
% ObjFunVal=sum((pxuIm(1,:)-pxuW(1,:)).^2) + sum((pxuIm(2,:)-pxuW(2,:)).^2) + 1e+4*(norm(q)-1)^2;
   
%% Objective function calculation (for e.g. lsqnonlin)
ObjFunVal=[(pxuIm(1,:)-pxuW(1,:)), (pxuIm(2,:)-pxuW(2,:)), 1e+4*(norm(q)-1)];
