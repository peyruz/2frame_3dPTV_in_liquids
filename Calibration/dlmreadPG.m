function result= dlmreadPG(filename,delimiter,r,c,range)

% I changed the code just slightly (Line 157) so tath it does not give error when one
% enters larger number of rows to be read than there actually is in the
% file. Now, one can read rows from R1 to the end of column using the range
% vector of [R1 C1 inf C2]
