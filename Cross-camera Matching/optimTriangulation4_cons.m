...
function [x3dAll,cumResnorm]=optimTriangulation4_cons(PolyCoef1x,PolyCoef1y,PolyCoef2x,PolyCoef2y, sz01, sz02,xi1,xi2,c1,c2,n,e3,dRF,sigmax1,sigmax2,sigmay1,sigmay2)
...
% for ii=1:size(sz01,2)
    ...
    optionsLsq = optimoptions(@fmincon,'Display','none',...
        'TypicalX', x3dinicur,...
        'MaxFunctionEvaluations',50000,'MaxIterations', 500,'StepTolerance',1e-10,'FunctionTolerance',1e-8,'UseParallel',false, 'ConstraintTolerance', 1e-3);
    
    f = @(x3d)optimTrianObjFun2_cons(x3d, c1, c2, xi1cur, xi2cur, n, PolyCoef1x,PolyCoef1y,PolyCoef2x,PolyCoef2y,dRF);
    
    searchMargin=0.05;
    lb=[x3dinicur(1)-searchMargin,x3dinicur(2)-searchMargin,x3dinicur(3)-4*searchMargin];
    ub=[x3dinicur(1)+searchMargin,x3dinicur(2)+searchMargin,x3dinicur(3)+4*searchMargin];
    
    % Constraints anonymous function
    fcon=@(x)nlcon(x,c1, c2, xi1, xi2, n, PolyCoef1x,PolyCoef1y,PolyCoef2x,PolyCoef2y, dRF, sigmax1,sigmay1,sigmax2,sigmay2);
    
    % Local optimization
%     [x3d,resnorm] = fmincon(f,x3dinicur,[],[],[],[],lb,ub,fcon,optionsLsq);

    % Global optimization
    ms = MultiStart('UseParallel',false);
    problem = createOptimProblem('fmincon','objective',...
                                f,'x0',x3dinicur,'lb',lb,'ub',ub,'nonlcon',fcon,'options',optionsLsq);    
%      problem = createOptimProblem('fmincon','objective',...
%                                 f,'x0',x3dinicur,'lb',lb,'ub',ub,'options',optionsLsq);
    NumOfStarts=20;    
    tic
    [x3d,fval,flag,output,solutions] = run(ms,problem,NumOfStarts);
    toc
                            
%                             [x3d,resnorm] = fmincon(f,x3dinicur,[],[],[],[],lb,ub,[],optionsLsq);
...
%% Objective function
function [ObjFunVal]=optimTrianObjFun2_cons(x3d,c1, c2, xi1, xi2, n, PolyCoef1x,PolyCoef1y,PolyCoef2x,PolyCoef2y, dRF)
...
ObjFunVal=  (PolyCalMatrix1*PolyCoef1x-xi1(1))^2 + (PolyCalMatrix1*PolyCoef1y-xi1(2))^2 + ...
            (PolyCalMatrix2*PolyCoef2x-xi2(1))^2 + (PolyCalMatrix2*PolyCoef2y-xi2(2))^2;

end

%% Nonlinear constraint function
function [c,ceq]=nlcon(x3d,c1, c2, xi1, xi2, n, PolyCoef1x,PolyCoef1y,PolyCoef2x,PolyCoef2y, dRF, sigmax1,sigmay1,sigmax2,sigmay2)
...
c=[ abs(PolyCalMatrix1*PolyCoef1x-xi1(1))-2*sigmax1;
    abs(PolyCalMatrix1*PolyCoef1y-xi1(2))-2*sigmay1;
    abs(PolyCalMatrix2*PolyCoef2x-xi2(1))-2*sigmax2;
    abs(PolyCalMatrix2*PolyCoef2y-xi2(2))-2*sigmay2];
