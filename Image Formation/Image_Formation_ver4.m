function [xi1, xi2]=Image_Formation_ver4(c1, c2, A1,A2, R1, R2, es13,es23, s,oi1,oi2,RInt,n)
...
% Radii defined based on undistorted coordinates
r1=sqrt(gs1(1,:).^2+gs1(2,:).^2);   
r2=sqrt(gs2(1,:).^2+gs2(2,:).^2);

% Add distortion
gsd1(1,:)=gs1(1,:)+gs1(1,:).*(k1*r1.^2+k2*r1.^4)+tau1*(r1.^2+2*gs1(1,:).^2)+2*tau2*gs1(1,:).*gs1(2,:)+A11*gs1(1,:)+B11*gs1(2,:);
gsd1(2,:)=gs1(2,:)+gs1(2,:).*(k1*r1.^2+k2*r1.^4)+tau2*(r1.^2+2*gs1(2,:).^2)+2*tau1*gs1(1,:).*gs1(2,:)+A12*gs1(2,:)+B12*gs1(1,:);

gsd2(1,:)=gs2(1,:)+gs2(1,:).*(k1*r2.^2+k2*r2.^4)+tau1*(r2.^2+2*gs2(1,:).^2)+2*tau2*gs2(1,:).*gs2(2,:)+A21*gs2(1,:)+B21*gs2(2,:);
gsd2(2,:)=gs2(2,:)+gs2(2,:).*(k1*r2.^2+k2*r2.^4)+tau2*(r2.^2+2*gs2(2,:).^2)+2*tau1*gs2(1,:).*gs2(2,:)+A22*gs2(2,:)+B22*gs2(1,:);
