...
%          'Smoothing'          -   [0,1,2,..] - number of smoothing
...
function [varargout]=detectParticles5_4(imPath, options, varargin)
...
for ii=1:imNum
    ...
    % Smoothing
    if ~isempty(options.Smoothing) && options.Smoothing>0
        for ss=1:options.Smoothing
    ...
    if ( (options.detectContours && nargout>=2) || (options.detectContours && options.write2file) )
        ...
        for kk=1:size(imStats,1)
            ...
            maxDotNum=1.85*pi*imStats.EquivDiameter(kk)/2;
